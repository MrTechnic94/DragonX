...
exports.run = async (client, message) => {
    ...
    return message.reply({embeds: [new EmbedBuilder().setDescription(`🎵 **Compressor został ${mode}!**`).setFooter({text: message.author.tag, iconURL: message.author.displayAvatarURL({dynamic: true})}).setColor(queue.filters.ffmpeg.isEnabled('compressor') ? `Green` : `Red`)]});
