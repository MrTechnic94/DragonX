...
exports.run = async (client, message, args) => {
    ...
    case 'queue':
        if (queue.repeatMode === QueueRepeatMode.QUEUE) return message.reply({embeds: [new EmbedBuilder().setDescription("❌ **Powtarzanie playlisty jest już włączone!**").setColor("Red")]});
        if (queue.repeatMode === QueueRepeatMode.OFF) await queue.setRepeatMode(QueueRepeatMode.QUEUE);
        message.reply({embeds: [new EmbedBuilder().setDescription("🔁 **Powtarzanie playlisty zostało włączone!**").setFooter({text: message.author.tag, iconURL: message.author.displayAvatarURL({dynamic: true})}).setColor("6b3deb")]});
        break;
    };
};
